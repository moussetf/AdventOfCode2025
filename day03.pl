...
parse([Bank|Banks]) -->
    ...
    {
        length(Digits, D), D > 1,
        maplist([X,Y]>>(Y is X-48), Digits, Bank)
    },
    blanks,
    parse(Banks).
parse([]) --> eos.

% Main predicate: U is the joltage for a list of battery ratings, if
% exactly K batteries can be turned on.
joltage(Bank, K, U) :-
...
% Auxiliary predicate operating on a list of pairs I-B, where B is
% the battery rating and I is the number of batteries coming after
% the current battery.
joltage_(Indexed, K, U) :-
    K > 0,
